...
function navHR(sub) {
  ...
  nav(sub === 'employees' ? 'employees' : sub === 'payrolls' ? 'payrolls' : 'orgchart');
...
function saveEmployee() {
  ...
  var data = {
    ...
    manager: document.getElementById('emp-manager').value.trim(),
...
function renderOrgChart() {
  ...
  if(!emps.length){
    el.innerHTML='<div style="color:var(--text3);padding:60px;font-size:13px;">No employees yet. Add employees first.</div>';
    return;
  }
  ...
  var companyName = DB.co.name || 'My Company';
  var companyIni  = (companyName||'C')[0].toUpperCase();

  var html = '<div style="display:flex;flex-direction:column;align-items:center;gap:0;font-family:\'DM Sans\',sans-serif;">';

  // Company root
  html += '<div style="background:var(--surface);border:2px solid var(--accent);border-radius:12px;padding:14px 24px;display:inline-flex;align-items:center;gap:10px;margin-bottom:0;">' +
  ...
  // Vertical connector
  html += '<div style="width:1px;height:30px;background:var(--border);margin:0 auto;"></div>';

  // Department row
  var deptKeys = Object.keys(depts);
  html += '<div style="display:flex;gap:20px;justify-content:center;position:relative;">';

  // Horizontal bar across departments
  if(deptKeys.length > 1) {
    html += '<div style="position:absolute;top:0;left:16px;right:16px;height:1px;background:var(--border);"></div>';
  }
  ...
  deptKeys.forEach(function(dept){
    ...
    // Vertical connector to dept box
    html += '<div style="width:1px;height:30px;background:var(--border);"></div>';
    // Dept box
    var dCol = empColor(dept);
    html += '<div style="background:var(--surface);border:1px dashed '+dCol+';border-radius:10px;padding:10px 18px;text-align:center;min-width:140px;margin-bottom:0;">' +
    ...
    members.forEach(function(e){
      ...
      html += '<div onclick="openEditEmployee('+e.id+')" style="background:var(--surface);border:1px solid var(--border);border-radius:10px;padding:12px 16px;display:flex;align-items:center;gap:10px;cursor:pointer;transition:all .15s;min-width:180px;" onmouseover="this.style.borderColor=\''+dCol+'\'" onmouseout="this.style.borderColor=\'var(--border)\'">' +
        ...
        '<div>' +
          '<div style="font-weight:600;font-size:12px;color:var(--text);">'+(e.fname||'')+' '+(e.lname||'')+'</div>' +
          '<div style="font-size:10px;color:var(--text3);">'+(e.position||dept)+'</div>' +
        '</div>' +
      '</div>';
    });
    html += '</div>';
...
function openNewPayroll() {
  ...
  sel.innerHTML = '<option value="">— Select Employee —</option>' +
    (DB.employees||[]).filter(function(e){ return e.status==='active'; }).map(function(e){
  ...
  document.getElementById('pr-period').value = period;
  ...
  document.getElementById('pr-ss-emp').value = '';
  document.getElementById('pr-irpf').value = '';
  document.getElementById('pr-ss-co').value = '';
  document.getElementById('pr-notes').value = '';

  // Show account codes
  var accs = getPayrollAccounts();
  document.getElementById('pr-acc-salary').textContent  = accs.salary.acc;
  document.getElementById('pr-acc-ss-emp').textContent  = accs.ssEmp.acc;
  document.getElementById('pr-acc-irpf').textContent    = accs.irpf.acc;
  document.getElementById('pr-acc-net').textContent     = accs.net.acc;
  document.getElementById('pr-acc-ss-co').textContent   = accs.ssCo.acc;
  document.getElementById('pr-acc-ss-total').textContent= accs.ssTotal.acc;
...
function prefillPayrollSalary() {
  ...
  var monthlyGross = (parseFloat(emp.salary)||0) / 12;
  document.getElementById('pr-gross').value = monthlyGross.toFixed(2);
  // Suggest SS employee ~6.35% and IRPF ~15% as defaults
  document.getElementById('pr-ss-emp').value = (monthlyGross * 0.0635).toFixed(2);
  document.getElementById('pr-irpf').value   = (monthlyGross * 0.15).toFixed(2);
  document.getElementById('pr-ss-co').value  = (monthlyGross * 0.2980).toFixed(2);
  calcPayroll();
}

function calcPayroll() {
  var gross  = parseFloat(document.getElementById('pr-gross').value)   || 0;
  var ssEmp  = parseFloat(document.getElementById('pr-ss-emp').value)  || 0;
  var irpf   = parseFloat(document.getElementById('pr-irpf').value)    || 0;
  var ssCo   = parseFloat(document.getElementById('pr-ss-co').value)   || 0;
  var net    = gross - ssEmp - irpf;
  var ssTotal = ssEmp + ssCo;

  var nd = document.getElementById('pr-net-display');   if(nd) nd.textContent=f(net);
  var st = document.getElementById('pr-ss-total-display'); if(st) st.textContent=f(ssTotal);
  ...
  var lines = [
    { side:'Dr', acc:accs.salary.acc, label:accs.salary.label, amt:gross },
    { side:'Dr', acc:accs.ssCo.acc,   label:accs.ssCo.label,   amt:ssCo  },
    { side:'Cr', acc:accs.ssTotal.acc,label:accs.ssTotal.label, amt:ssTotal },
    { side:'Cr', acc:accs.irpf.acc,   label:accs.irpf.label,   amt:irpf  },
    { side:'Cr', acc:accs.net.acc,    label:accs.net.label,     amt:net   },
  ].filter(function(l){ return l.amt > 0.001; });

  prev.innerHTML = '<table style="width:100%;border-collapse:collapse;font-size:11px;">' +
    '<tr style="border-bottom:1px solid var(--border);"><th style="text-align:left;padding:4px 8px;color:var(--text3);font-family:monospace;">Side</th><th style="text-align:left;padding:4px 8px;color:var(--text3);font-family:monospace;">Account</th><th style="text-align:right;padding:4px 8px;color:var(--text3);font-family:monospace;">Amount</th></tr>' +
    lines.map(function(l){
      return '<tr style="border-bottom:1px solid rgba(46,46,56,.3);">' +
        '<td style="padding:4px 8px;font-weight:700;color:'+(l.side==='Dr'?'var(--green)':'var(--red)')+';">'+l.side+'</td>' +
        '<td style="padding:4px 8px;font-family:monospace;color:var(--text2);">'+l.acc+' <span style="color:var(--text3);">'+l.label+'</span></td>' +
        '<td style="padding:4px 8px;text-align:right;font-family:monospace;font-weight:600;">'+f(l.amt)+'</td>' +
      '</tr>';
    }).join('') +
    '<tr style="border-top:2px solid var(--border);font-weight:700;">' +
      '<td colspan="2" style="padding:6px 8px;color:var(--text);">Total Debit / Credit</td>' +
      '<td style="padding:6px 8px;text-align:right;font-family:monospace;color:var(--accent);">'+f(gross+ssCo)+' / '+f(gross+ssCo)+'</td>' +
    '</tr>' +
  '</table>';
}

function openEditPayroll(id) {
  // For now just show a read-only view — full edit TBD
...
function savePayroll() {
  ...
  // Build Journal Entry
  var jeLines = [];
  if(gross > 0.001)   jeLines.push({ account:accs.salary.acc,  debit:gross,   credit:0 });
  if(ssCo  > 0.001)   jeLines.push({ account:accs.ssCo.acc,    debit:ssCo,    credit:0 });
  if(ssTotal>0.001)   jeLines.push({ account:accs.ssTotal.acc,  debit:0,       credit:ssTotal });
  if(irpf  > 0.001)   jeLines.push({ account:accs.irpf.acc,    debit:0,       credit:irpf });
  if(net   > 0.001)   jeLines.push({ account:accs.net.acc,      debit:0,       credit:net });

  var jeId = DB.ids.j++;
  DB.je.push({
    id: jeId, date: paydate,
    desc: 'Payroll — '+empName+' / '+period+(type!=='monthly'?' ('+type+')':''),
    lines: jeLines,
    amount: gross+ssCo,
    auto: true, sourceType: 'payroll',
    empId: empId, payrollPeriod: period
  });

  var prId = DB.ids.pr++;
  DB.payrolls.push({
    id: prId, empId: empId, empName: empName,
    period: period, paydate: paydate, type: type,
    gross: gross, ssEmp: ssEmp, ssCo: ssCo,
    ssTotal: ssTotal, irpf: irpf, net: net,
    jeId: jeId, notes: notes
  });

  sv(); closeOverlay('ov-payroll'); rPayrolls(); renderAll();
  showToast('✅ Payroll posted — '+empName+' · '+period+' · Net: '+f(net)+' · JE created');
}



function renderAll(){rDash();rContacts();rSales();rRecurring();rPurch();rColl();rPay();rJE();rPL();rBS();rCF();rCFO();rBankAccounts();rEmployees();rPayrolls();}
        ...
        <div class="mbody" style="max-height:70vh;overflow-y:auto;">
          ...
          <div id="epanel-contract" style="display:none;">
            <div class="fgrid">
              ...
              <div class="fg"><label>Reports To (Manager)</label><input type="text" id="emp-manager" placeholder="Employee name or ID"></div>
            </div>
          </div>
          ...
          <div class="card" style="padding:0;overflow:hidden;margin-bottom:16px;">
            <table style="width:100%;border-collapse:collapse;">
              <thead><tr style="background:var(--surface2);">
                ...
                <th style="padding:8px 12px;text-align:right;font-size:11px;color:var(--text3);font-family:monospace;text-transform:uppercase;">Account</th>
                <th style="padding:8px 12px;text-align:right;font-size:11px;color:var(--text3);font-family:monospace;text-transform:uppercase;">Amount (€)</th>
              </tr></thead>
              <tbody>
                <tr style="border-bottom:1px solid var(--border);">
                  <td style="padding:8px 12px;font-size:13px;color:var(--text);">Gross Salary</td>
                  <td style="padding:8px 12px;text-align:right;font-family:monospace;font-size:11px;color:var(--text3);" id="pr-acc-salary"></td>
                  <td style="padding:8px 12px;"><input type="number" id="pr-gross" placeholder="0.00" step="0.01" oninput="calcPayroll()" style="width:100%;text-align:right;padding:4px 8px;background:var(--surface2);border:1px solid var(--border);border-radius:6px;color:var(--text);font-size:13px;"></td>
                </tr>
                <tr style="border-bottom:1px solid var(--border);">
                  <td style="padding:8px 12px;font-size:13px;color:var(--text);">SS Employee Contribution (−)</td>
                  <td style="padding:8px 12px;text-align:right;font-family:monospace;font-size:11px;color:var(--text3);" id="pr-acc-ss-emp"></td>
                  <td style="padding:8px 12px;"><input type="number" id="pr-ss-emp" placeholder="0.00" step="0.01" oninput="calcPayroll()" style="width:100%;text-align:right;padding:4px 8px;background:var(--surface2);border:1px solid var(--border);border-radius:6px;color:var(--text);font-size:13px;"></td>
                </tr>
                <tr style="border-bottom:1px solid var(--border);">
                  <td style="padding:8px 12px;font-size:13px;color:var(--text);">IRPF Withholding (−)</td>
                  <td style="padding:8px 12px;text-align:right;font-family:monospace;font-size:11px;color:var(--text3);" id="pr-acc-irpf"></td>
                  <td style="padding:8px 12px;"><input type="number" id="pr-irpf" placeholder="0.00" step="0.01" oninput="calcPayroll()" style="width:100%;text-align:right;padding:4px 8px;background:var(--surface2);border:1px solid var(--border);border-radius:6px;color:var(--text);font-size:13px;"></td>
                </tr>
                <tr style="border-bottom:1px solid var(--border);background:rgba(200,255,0,.03);">
                  <td style="padding:8px 12px;font-size:13px;font-weight:700;color:var(--text);">Net Salary (to pay employee)</td>
                  <td style="padding:8px 12px;text-align:right;font-family:monospace;font-size:11px;color:var(--text3);" id="pr-acc-net"></td>
                  <td style="padding:8px 12px;text-align:right;font-family:'DM Serif Display',serif;font-size:18px;color:var(--accent);" id="pr-net-display">€0.00</td>
                </tr>
                <tr style="border-bottom:1px solid var(--border);">
                  <td style="padding:8px 12px;font-size:13px;color:var(--text);">SS Company Cost (+)</td>
                  <td style="padding:8px 12px;text-align:right;font-family:monospace;font-size:11px;color:var(--text3);" id="pr-acc-ss-co"></td>
                  <td style="padding:8px 12px;"><input type="number" id="pr-ss-co" placeholder="0.00" step="0.01" oninput="calcPayroll()" style="width:100%;text-align:right;padding:4px 8px;background:var(--surface2);border:1px solid var(--border);border-radius:6px;color:var(--text);font-size:13px;"></td>
                </tr>
                <tr>
                  <td style="padding:8px 12px;font-size:13px;color:var(--text);">Total SS Payable (Employee + Company)</td>
                  <td style="padding:8px 12px;text-align:right;font-family:monospace;font-size:11px;color:var(--text3);" id="pr-acc-ss-total"></td>
                  <td style="padding:8px 12px;text-align:right;font-family:monospace;font-size:13px;font-weight:700;color:var(--yellow);" id="pr-ss-total-display">€0.00</td>
                </tr>
              </tbody>
            </table>
          </div>
...
<style>
#ao{position:fixed;inset:0;background:#0f0f11;z-index:9999;display:flex;align-items:center;justify-content:center;}
#ao.hidden{display:none;}
.ab{background:#16161a;border:1px solid #2e2e38;border-radius:16px;padding:40px;width:min(420px,94vw);}
.al{font-family:'DM Serif Display',serif;font-size:32px;color:#c8ff00;margin-bottom:4px;}
.as{font-size:11px;color:#5a5a72;font-family:monospace;text-transform:uppercase;letter-spacing:2px;margin-bottom:28px;}
.at{font-size:18px;font-weight:700;color:#f0f0f4;margin-bottom:18px;}
.af{display:flex;flex-direction:column;gap:5px;margin-bottom:12px;}
.af label{font-size:11px;color:#5a5a72;font-family:monospace;text-transform:uppercase;letter-spacing:1px;}
.af input{background:#1e1e24;border:1px solid #2e2e38;border-radius:8px;padding:10px 14px;color:#f0f0f4;font-size:14px;outline:none;}
.af input:focus{border-color:#c8ff00;}
.ab-btn{width:100%;padding:12px;background:#c8ff00;color:#0f0f11;border:none;border-radius:8px;font-size:14px;font-weight:700;cursor:pointer;margin-top:6px;}
.ab-btn.sec{background:transparent;color:#9090a8;border:1px solid #2e2e38;margin-top:4px;}
.ae{color:#f87171;font-size:12px;margin-top:6px;min-height:16px;text-align:center;}
.aok{color:#4ade80;font-size:12px;margin-top:6px;min-height:16px;text-align:center;}
.sep{display:flex;align-items:center;gap:10px;margin:14px 0;color:#5a5a72;font-size:12px;}
.sep::before,.sep::after{content:'';flex:1;height:1px;background:#2e2e38;}
</style>
<div id="ao"><div class="ab">
  <div class="al">Leadger</div><div class="as">Accounting System</div>
  <div id="a-login">
    <div class="at">Welcome back</div>
    <div class="af"><label>Email</label><input type="email" id="a-em" placeholder="you@company.com" onkeydown="if(event.key==='Enter')aLogin()"></div>
    <div class="af"><label>Password</label><input type="password" id="a-pw" placeholder="••••••••" onkeydown="if(event.key==='Enter')aLogin()"></div>
    <div class="ae" id="a-err"></div>
    <button class="ab-btn" onclick="aLogin()">Sign In</button>
    <div class="sep">or</div>
    <button class="ab-btn sec" onclick="aShow('register')">Create an account</button>
    <button class="ab-btn sec" onclick="aShow('reset')" style="margin-top:4px;font-size:12px;border:none;color:#5a5a72;">Forgot password?</button>
  </div>
  <div id="a-register" style="display:none;">
    <div class="at">Create account</div>
    <div class="af"><label>Email</label><input type="email" id="r-em" placeholder="you@company.com"></div>
    <div class="af"><label>Password</label><input type="password" id="r-pw" placeholder="Min 6 characters"></div>
    <div class="af"><label>Confirm</label><input type="password" id="r-pw2" onkeydown="if(event.key==='Enter')aRegister()"></div>
    <div class="ae" id="r-err"></div>
    <button class="ab-btn" onclick="aRegister()">Create Account</button>
    <div class="sep">or</div>
    <button class="ab-btn sec" onclick="aShow('login')">Already have an account?</button>
  </div>
  <div id="a-reset" style="display:none;">
    <div class="at">Reset password</div>
    <div class="af"><label>Email</label><input type="email" id="rs-em" placeholder="you@company.com"></div>
    <div class="ae" id="rs-err"></div><div class="aok" id="rs-ok"></div>
    <button class="ab-btn" onclick="aReset()">Send Reset Email</button>
    <div class="sep">or</div>
    <button class="ab-btn sec" onclick="aShow('login')">Back to sign in</button>
  </div>
</div></div>
<script src="https://cdn.jsdelivr.net/npm/@supabase/supabase-js@2"></script>
<script>
const _SU='https://xoaujnyamjsexybjswwj.supabase.co',_SK='sb_publishable_BcWxOGS-mTzTimjQZvFSIw_f0kyY1ka';
const _sp=supabase.createClient(_SU,_SK);var _cu=null;
function aShow(p){['login','register','reset'].forEach(function(x){document.getElementById('a-'+x).style.display=x===p?'':'none';});['a-err','r-err','rs-err','rs-ok'].forEach(function(id){var el=document.getElementById(id);if(el)el.textContent='';});}
async function aLogin(){var em=document.getElementById('a-em').value.trim(),pw=document.getElementById('a-pw').value,el=document.getElementById('a-err');el.textContent='Signing in...';var{data,error}=await _sp.auth.signInWithPassword({email:em,password:pw});if(error){el.textContent=error.message;return;}_cu=data.user;aSuccess();}
async function aRegister(){var em=document.getElementById('r-em').value.trim(),pw=document.getElementById('r-pw').value,pw2=document.getElementById('r-pw2').value,el=document.getElementById('r-err');if(!em||!pw){el.textContent='Required.';return;}if(pw!==pw2){el.textContent='Passwords do not match.';return;}if(pw.length<6){el.textContent='Min 6 chars.';return;}el.textContent='Creating...';var{data,error}=await _sp.auth.signUp({email:em,password:pw});if(error){el.textContent=error.message;return;}_cu=data.user;var{data:d2,error:e2}=await _sp.auth.signInWithPassword({email:em,password:pw});if(!e2&&d2.user){_cu=d2.user;aSuccess();}else{el.style.color='#4ade80';el.textContent='Check email, then sign in.';aShow('login');}
}
async function aReset(){var em=document.getElementById('rs-em').value.trim(),el=document.getElementById('rs-err'),ok=document.getElementById('rs-ok');if(!em){el.textContent='Enter email.';return;}var{error}=await _sp.auth.resetPasswordForEmail(em);if(error){el.textContent=error.message;return;}ok.textContent='Reset sent!';}
function aSuccess(){document.getElementById('ao').classList.add('hidden');var s=(_cu.email||'').split('@')[0];var g=document.getElementById('topGreeting');if(g)g.textContent=s;loadFromSupabase();}
async function saveToSupabase(d){if(!_cu)return;await _sp.from('user_data').upsert({user_id:_cu.id,data:d,updated_at:new Date().toISOString()},{onConflict:'user_id'});}
async function loadFromSupabase(){if(!_cu)return;var{data,error}=await _sp.from('user_data').select('data').eq('user_id',_cu.id).single();if(!error&&data&&data.data&&data.data.ids){DB=data.data;refreshCOA();var af=['wires','contacts','recurring','assets','deprPlans','bankAccounts','bankMovements','employees','payrolls','taxes','paymentMethods'];af.forEach(function(f){if(!DB[f])DB[f]=[];});var ki={wt:1,ba:1,bm:1,em:1,pr:1};Object.keys(ki).forEach(function(k){if(!DB.ids[k])DB.ids[k]=ki[k];});}updateUserUI();renderAll();initJE();}
var _os=sv;sv=function(){_os();saveToSupabase(DB);};
async function signOut(){closeUserDropdown();await _sp.auth.signOut();_cu=null;document.getElementById('ao').classList.remove('hidden');aShow('login');}
(async function(){var{data:{session}}=await _sp.auth.getSession();if(session&&session.user){_cu=session.user;aSuccess();}_sp.auth.onAuthStateChange(function(ev,s){if(ev==='SIGNED_IN'&&s){_cu=s.user;aSuccess();}if(ev==='SIGNED_OUT'){_cu=null;document.getElementById('ao').classList.remove('hidden');}});})();
</script></body>
</html>"""
...
def main():
    ...
    if HAS_WEBVIEW:
        ...
        print()
        print("╔══════════════════════════════════════════════════════════╗")
        print("║       FinLedger — Professional Accounting System         ║")
        print("╚══════════════════════════════════════════════════════════╝")
        print()
        print("  AVISO: pywebview no está instalado.")
...
<meta charset="UTF-8">
<meta name="viewport" content="width=device-width, initial-scale=1.0">
<meta name="description" content="Leadger — Professional accounting software for modern businesses. Double-entry bookkeeping, VAT management, payroll, treasury and CFO analytics. Start free.">
<meta name="keywords" content="accounting software, software contabilidad, bookkeeping, VAT, IVA, payroll, treasury, CFO, PGC España, US GAAP">
<meta property="og:title" content="Leadger — Professional Accounting Software">
<meta property="og:description" content="Double-entry bookkeeping, VAT, payroll and CFO analytics for modern businesses.">
<meta property="og:url" content="https://leadger.net">
<meta property="og:type" content="website">
<link rel="canonical" href="https://leadger.net">
<title>Leadger — Professional Accounting Software</title>
...
<style>
  ...
  *, *::before, *::after { box-sizing: border-box; margin: 0; padding: 0; }
  ...
  .nav-cta:hover { background: #d4ff26 !important; }
  ...
  .app-logo-mock span { color: var(--accent); }
  ...
  .kpi-label { font-size: 9px; color: var(--text3); font-family: 'DM Mono', monospace; text-transform: uppercase; letter-spacing: 1px; margin-bottom: 6px; }
  .kpi-val { font-family: 'Instrument Serif', serif; font-size: 20px; }
  ...
  .plan-btn:hover { border-color: var(--text3); color: var(--text); }
  ...
  .badge-card .badge-icon { font-size: 28px; margin-bottom: 8px; }
  .badge-card .badge-title { font-size: 13px; font-weight: 600; color: var(--text); margin-bottom: 4px; }
  .badge-card .badge-desc { font-size: 11px; color: var(--text3); }
  ...
  .cta-section h2 { margin: 0 auto 20px; text-align: center; }
  ...
  @media (max-width: 768px) {
    ...
    .plans-wrap { padding: 64px 24px; }
    .stats-wrap { padding: 48px 24px; }
    .compliance-wrap { padding: 64px 24px; }
...
<!-- NAV -->
<nav id="navbar">
  <div class="nav-logo"><img src="data:image/svg+xml;base64," alt="Leadger" style="height:32px;"></div>
  <ul class="nav-links">
    <li><a href="#features">Features</a></li>
    <li><a href="#plans">Pricing</a></li>
    <li><a href="#compliance">Compliance</a></li>
    <li><a href="https://app.leadger.net" class="nav-cta">Start free →</a></li>
  </ul>
</nav>
...
<div class="hero">
  ...
  <h1>Accounting that<br><em>actually works</em><br>for your business</h1>

  <p class="hero-sub">
    Double-entry bookkeeping, VAT management, payroll, treasury reconciliation
    and CFO analytics — all in one platform built for modern finance teams.
  </p>

  <div class="hero-ctas">
    <a href="https://app.leadger.net" class="btn-primary">Start for free →</a>
  ...
  <div class="preview-wrap">
    ...
    <div class="preview-frame">
      <div class="preview-bar">
        ...
        <div class="preview-url">app.leadger.net</div>
      </div>
      <div class="app-mockup">
        <div class="app-sidebar">
          <div class="app-logo-mock"><img src="data:image/svg+xml;base64," alt="Leadger" style="height:18px;"></div>
          <div class="sidebar-section">Overview</div>
          <div class="sidebar-item active"><span class="icon">◈</span>Dashboard</div>
          <div class="sidebar-section">Operations</div>
          <div class="sidebar-item"><span class="icon">🧾</span>Sales</div>
          <div class="sidebar-item"><span class="icon">🛒</span>Purchases</div>
          <div class="sidebar-item"><span class="icon">👥</span>Contacts</div>
          <div class="sidebar-section">Treasury</div>
          <div class="sidebar-item"><span class="icon">🏦</span>Bank Accounts</div>
          <div class="sidebar-item"><span class="icon">⇄</span>Wire Transfers</div>
          <div class="sidebar-section">Reports</div>
          <div class="sidebar-item"><span class="icon">📊</span>Taxes</div>
          <div class="sidebar-item"><span class="icon">📈</span>P&L Statement</div>
          <div class="sidebar-item"><span class="icon">★</span>CFO Module</div>
        </div>
        <div class="app-content">
          <div class="content-header">
            <div class="content-title">Dashboard</div>
            <div class="mock-btn">+ New Invoice</div>
          </div>
          <div class="kpi-row">
            <div class="kpi-card">
              <div class="kpi-label">Revenue</div>
              <div class="kpi-val green">€248k</div>
            </div>
            <div class="kpi-card">
              <div class="kpi-label">Expenses</div>
              <div class="kpi-val red">€91k</div>
            </div>
            <div class="kpi-card">
              <div class="kpi-label">Net Income</div>
              <div class="kpi-val accent">€157k</div>
            </div>
            <div class="kpi-card">
              <div class="kpi-label">Cash</div>
              <div class="kpi-val" style="color:var(--text)">€83k</div>
            </div>
          </div>
          <div class="chart-mock">
            <div class="bar-mock" style="height:30%;background:var(--green)"></div>
            <div class="bar-mock" style="height:50%;background:var(--green)"></div>
            <div class="bar-mock" style="height:40%;background:var(--green)"></div>
            <div class="bar-mock" style="height:70%;background:var(--green)"></div>
            <div class="bar-mock" style="height:55%;background:var(--green)"></div>
            <div class="bar-mock" style="height:85%;background:var(--green)"></div>
            <div class="bar-mock" style="height:65%;background:var(--green)"></div>
            <div class="bar-mock" style="height:90%;background:var(--green)"></div>
            <div class="bar-mock" style="height:25%;background:var(--red)"></div>
            <div class="bar-mock" style="height:35%;background:var(--red)"></div>
            <div class="bar-mock" style="height:30%;background:var(--red)"></div>
            <div class="bar-mock" style="height:45%;background:var(--red)"></div>
            <div class="bar-mock" style="height:40%;background:var(--red)"></div>
            <div class="bar-mock" style="height:55%;background:var(--red)"></div>
            <div class="bar-mock" style="height:38%;background:var(--red)"></div>
            <div class="bar-mock" style="height:42%;background:var(--red)"></div>
          </div>
        </div>
      </div>
    </div>
...
<div class="stats-wrap">
  <div class="stats-inner">
    <div class="reveal">
      ...
      <div class="stat-label">Double-entry accuracy</div>
    </div>
    <div class="reveal">
      <div class="stat-val">2</div>
      <div class="stat-label">Accounting plans (PGC &amp; GAAP)</div>
    </div>
    <div class="reveal">
      <div class="stat-val">∞</div>
      <div class="stat-label">Users per account</div>
    </div>
    <div class="reveal">
      <div class="stat-val">0€</div>
      <div class="stat-label">To start today</div>
    </div>
  </div>
</div>
...
<section id="features">
  ...
  <h2>Everything your<br>finance team <em>needs</em></h2>
  <p class="section-sub">From daily invoicing to CFO-level analytics — Leadger covers the full accounting lifecycle without the complexity.</p>

  <div class="features-grid reveal">
    <div class="feature-card">
      <div class="feature-icon">🧾</div>
      <div class="feature-title">Sales &amp; Purchases</div>
      <div class="feature-desc">Create invoices with IVA and retención, manage purchase orders, and auto-generate journal entries. Supports PGC España and US GAAP simultaneously.</div>
    </div>
    <div class="feature-card">
      <div class="feature-icon">🏦</div>
      <div class="feature-title">Treasury &amp; Reconciliation</div>
      <div class="feature-desc">Import bank statements, reconcile movements against invoices or remittances, and manage wire transfers. Real-time cash position from Journal Entries.</div>
    </div>
    <div class="feature-card">
      <div class="feature-icon">📒</div>
      <div class="feature-title">Double-Entry Bookkeeping</div>
      <div class="feature-desc">Every transaction generates a balanced journal entry automatically. Full audit trail, drill-down from P&amp;L to individual accounts.</div>
    </div>
    <div class="feature-card">
      <div class="feature-icon">📊</div>
      <div class="feature-title">Tax Management</div>
      <div class="feature-desc">Track IVA repercutido, IVA soportado, and retenciones by quarter. Quarterly summary ready for Modelo 303, 111, and 115 preparation.</div>
    </div>
    <div class="feature-card">
      <div class="feature-icon">👤</div>
      <div class="feature-title">HR &amp; Payroll</div>
      <div class="feature-desc">Manage employees, generate payrolls with SS, IRPF and net calculations, and post directly to journal entries. Org chart with reporting hierarchy.</div>
    </div>
    <div class="feature-card">
      <div class="feature-icon">★</div>
      <div class="feature-title">CFO Intelligence</div>
      <div class="feature-desc">Financial ratios, cash flow waterfall, patrimonial structure pie chart, customer cohort analysis and revenue vs. net result trends.</div>
    </div>
  </div>
</section>

<!-- PLANS -->
<div class="plans-wrap" id="plans">
  <div class="plans-inner">
    <div class="section-label">Pricing</div>
    <h2>Simple pricing,<br><em>no surprises</em></h2>
    <p class="section-sub">Start free and upgrade when your business grows. All plans include full access to core accounting features.</p>

    <div class="plans-grid reveal">
      <div class="plan-card">
        <div class="plan-name">Starter</div>
        <div class="plan-price">9<span style="font-size:28px;font-family:'DM Sans',sans-serif;font-weight:600;">,99€</span></div>
        <div class="plan-period">per month · web hosted</div>
        <ul class="plan-features">
          <li>Sales &amp; Purchases</li>
          <li>Journal Entries</li>
          <li>P&amp;L · BS · Cash Flow</li>
      ...
      <div class="plan-card featured">
        ...
        <div class="plan-price">29<span>€</span></div>
        <div class="plan-period">per month · billed monthly</div>
        <ul class="plan-features">
          <li>Unlimited invoices</li>
          <li>Treasury &amp; Reconciliation</li>
          <li>Wire Transfers &amp; Remittances</li>
          <li>Tax module (IVA &amp; Retenciones)</li>
          <li>HR &amp; Payroll</li>
          <li>CFO Intelligence module</li>
          <li>Unlimited bank accounts</li>
          <li>Priority support</li>
        </ul>
        <a href="https://app.leadger.net" class="plan-btn featured-btn">Start free trial →</a>
      </div>

      <div class="plan-card">
        <div class="plan-name">CFO Premium</div>
        <div class="plan-price">79<span>€</span></div>
        <div class="plan-period">per month · or €690/yr</div>
        <ul class="plan-features">
          <li>All Professional features</li>
          <li>Financial ratio dashboard</li>
          <li>KPI tracker &amp; MRR/ARR</li>
          <li>Cohort analysis</li>
          <li>AI forecasting &amp; scenarios</li>
          <li>Power BI connector</li>
        </ul>
        <a href="mailto:hello@leadger.net" class="plan-btn" style="background:var(--accent);color:#080808;border-color:transparent;">High-value upsell →</a>
      </div>
    </div>
  </div>
</div>
...
<!-- COMPLIANCE -->
<div class="compliance-wrap" id="compliance">
  <div class="compliance-inner">
    <div class="reveal">
      <div class="section-label">Compliance</div>
      <h2>Built for <em>Spanish</em><br>&amp; international<br>businesses</h2>
      <p class="section-sub" style="margin-bottom:0;">Leadger supports both PGC España (Plan General Contable) and US GAAP, switchable at any time without losing data.</p>
    </div>
    <div class="compliance-badges reveal">
      <div class="badge-card">
        <div class="badge-icon">🇪🇸</div>
        <div class="badge-title">PGC España</div>
        <div class="badge-desc">Cuentas 100–778, IVA 21%/10%/4%, Retenciones, Modelo 303</div>
      </div>
      <div class="badge-card">
        <div class="badge-icon">🇺🇸</div>
        <div class="badge-title">US GAAP</div>
        <div class="badge-desc">Accounts 1000–7100, FICA, withholding tax, standard reports</div>
      </div>
      <div class="badge-card">
        <div class="badge-icon">🔒</div>
        <div class="badge-title">GDPR Ready</div>
        <div class="badge-desc">Data hosted in Frankfurt, EU. Per-user encryption via Supabase.</div>
      </div>
      <div class="badge-card">
        <div class="badge-icon">📋</div>
        <div class="badge-title">Audit Trail</div>
        <div class="badge-desc">Every entry logged. Full double-entry integrity guaranteed.</div>
      </div>
    </div>
  </div>
</div>
...
<div class="cta-section">
  ...
  <h2>Your accounts,<br><em>finally in order</em></h2>
  <p class="section-sub">Join businesses using Leadger to manage their accounting, payroll and treasury in one place. No credit card required.</p>
  <div class="hero-ctas">
    <a href="https://app.leadger.net" class="btn-primary">Create free account →</a>
    <a href="mailto:hello@leadger.net" class="btn-secondary">Talk to us</a>
  </div>
</div>
...
<footer>
  <div class="footer-inner">
    <div class="footer-brand">
      ...
      <p>Professional accounting software for modern businesses. Built with precision for finance teams.</p>
    </div>
    <div class="footer-col">
      ...
      <ul>
        ...
        <li><a href="#plans">Pricing</a></li>
        <li><a href="#compliance">Compliance</a></li>
        <li><a href="https://app.leadger.net">Login</a></li>
      </ul>
    </div>
    <div class="footer-col">
      <h4>Company</h4>
      <ul>
        ...
        <li><a href="mailto:hello@leadger.net">Contact</a></li>
        <li><a href="#">Blog</a></li>
      </ul>
    </div>
    <div class="footer-col">
  ...
  <div class="footer-bottom">
    <p>© 2026 Leadger. All rights reserved.</p>
    <p style="font-family:'DM Mono',monospace;font-size:11px;color:var(--text3);">Built on Railway · Supabase · Frankfurt, EU</p>
  </div>
</footer>
...
class H(http.server.BaseHTTPRequestHandler):
    ...
    def _e(self):
        try:
            ...
            payload=json.loads(body);content=payload.get('content','');fname=payload.get('filename','export.csv')
